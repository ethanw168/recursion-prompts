...
var exponent = function(base, exp) {
  ...
  if (absexp % 2 === 0) {
    var y = exponent(base, absexp / 2);
    return exp < 0 ? 1 / (y * y) : y * y;
  } else {
    var y = base * exponent(base, absexp - 1);
    return exp < 0 ? 1 / y : y;
  }
};
...
var countValuesInObj = function(obj, value) {
  ...
  for (var objkey in obj) {
    if (typeof obj[objkey] === 'object') {
      count += countValuesInObj(obj[objkey], value);
    } else {
      count += obj[objkey] === value ? 1 : 0;
    }
  }
  return count;
};
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
  ...
  for (var objkey in obj) {
    if (typeof obj[objkey] === 'object') {
      replaceKeysInObj(obj[objkey], oldKey, newKey);
    }
  }
  return obj;
};
